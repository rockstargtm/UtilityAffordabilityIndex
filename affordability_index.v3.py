...
def get_census_url(year: int, geography: str) -> str:
    ...
    if geography == "county":
        return f"https://api.census.gov/data/{year}/acs/acs5/subject?get=NAME,S1701_C03_001E&for=county:*&in=state:*"
    else:  # state
...
def load_eia_sales_data(filepath: str) -> pd.DataFrame:
    ...
    # Map columns based on the actual format
    # Looking for: RESIDENTIAL_Revenues, RESIDENTIAL_Sales
    # And: Utility Characteristics_Unnamed: 0, 1, 2, etc.
    
    util_char_cols = sorted([col for col in df.columns if 'Utility Characteristics' in col])
    
    col_map = {}
    
    # Try to find revenue and sales (these are easier)
    for col in df.columns:
        col_upper = col.upper()
    ...
    # For utility characteristics, inspect the data
    if util_char_cols:
...
def load_service_territory_data(filepath: str) -> Optional[pd.DataFrame]:
    ...
    try:
        xls = pd.ExcelFile(filepath)
        sheet_name = None
        for name in ["Service_Territory", "Service Territory", "Data"]:
            if name in xls.sheet_names:
                sheet_name = name
                break
        
        if not sheet_name:
            print("⚠ Could not find territory sheet")
            return None
        ...
        for col in df.columns:
            col_upper = col.upper()
            if 'UTILITY' in col_upper and ('NUMBER' in col_upper or 'ID' in col_upper):
                utility_col = col
            elif 'FIPS' in col_upper or ('COUNTY' in col_upper and 'CODE' in col_upper):
                fips_col = col
            elif any(kw in col_upper for kw in ['CUSTOMER', 'PERCENT', 'SHARE', 'PCT']):
        ...
        df['fips_code'] = df['fips_code'].astype(str).str.strip().str.replace('.0', '', regex=False).str.zfill(5)
...
def fetch_county_poverty() -> Optional[pd.DataFrame]:
    ...
    for year in ACS_YEARS:
        try:
            ...
            if response.status_code == 200:
                ...
                df = df[df['poverty_rate_percent'].notna() & (df['poverty_rate_percent'] >= 0) & (df['poverty_rate_percent'] <= 100)].copy()
    ...
    print(f"✗ Failed to fetch county data from all years")
    return None
...
def fetch_state_poverty() -> pd.DataFrame:
    ...
    # First, check for uploaded Census ACS files (5-year estimates have state data)
    census_upload_files = [
    ...
    for filepath in census_upload_files:
        if os.path.exists(filepath):
            try:
                ...
                # Find the poverty rate column (S1701_C03_001E)
    ...
    # Third, fallback to local CSV files
    ...
    # If nothing works, provide clear instructions
    print(f"\n✗ ERROR: Could not load state poverty data")
    print(f"\n📥 SOLUTION: Use one of the state_poverty CSV files from the outputs directory")
    print(f"   Copy it to your working directory and re-run.")
    sys.exit(1)
...
def clean_sales_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df[
        ...
        df['residential_sales_mwh'].notna() & (df['residential_sales_mwh'] > 0)
    ].copy()
    
    print(f"Valid data: {len(df):,}/{initial:,} rows")
    return df
...
def compute_rates(df: pd.DataFrame, keep_outliers: bool = False) -> pd.DataFrame:
    ...
    print(f"Rate stats: ${df['electricity_rate_usd_per_kwh'].min():.4f} - ${df['electricity_rate_usd_per_kwh'].max():.4f}")
